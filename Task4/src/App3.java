...
public class App3 {
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            System.out.println("1: f1(x) = x + 1");
            System.out.println("2: f2(x) = x^2 + 2x + 2");
            System.out.println("3: f3(x) = x^3 + 3 x^2 + 3x + 3");
            ...
            for (int i = 0; i < 5; i++) {
                ...
                System.out.printf("%-18s: J(h) = %.15f  Абс. погр. = %.15f  Отн. погр. = %.15f %%\n", 
            ...
            if (rungeChoice == 1) {
                ...
                // Вывод таблицы результатов уточнения0
                System.out.printf("  %-20s   %-18s   %-18s   %-18s   %-18s   %-18s   %-18s  \n", 
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    System.out.printf("  %-18s   %.15f   %.15f   %.15f   %.15f   %.15f   %.15f  \n",
    ...
    private static double f(int idx, double x) {
        switch (idx) {
            case 0: return 1.0;
            case 1: return x + 1;
            case 2: return x * x + 2 * x + 2;
            case 3: return x * x * x + 3 * x * x + 3 * x + 3;
    ...
    private static double integral(int idx, double a, double b) {
        switch (idx) {
            case 0: return (b - a);
            case 1: return (b * b - a * a) / 2.0 + (b - a);
            case 2: return (b * b * b - a * a * a) / 3.0 + 2 * (b * b - a * a) / 2.0 + 2 * (b - a);
            case 3: return (b * b * b * b - a * a * a * a) / 4.0 + 3 *(b * b * b - a * a * a) / 3.0 + 3 * (b * b - a * a) / 2.0 + 3 * (b - a);
    ...
    private static double quadSimp(int idx, double a, double b, int m) {
        ...
        double sum = 0;
        for (int i = 1; i <= m; i++) {
            sum += h / 6.0 * (f(idx, a + ((i - 1) * h)) + 4 * f(idx, a + ((2 * i - 1) / 2.0 * h)) + f(idx, a + (i * h)));
        }
        return sum;
    }
}
